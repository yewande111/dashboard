		   ...
		   $query = mysqli_query($connect, "SELECT * FROM courses") or die(mysqli_error());
		    if (mysqli_num_rows($query) > 0 ) {
		        ...
		        while($fetch = mysqli_fetch_array($query)) {
		        	$id = $fetch['id'];
		            echo "<tr><td>".($i+1)."</td><td>".$fetch['course_name']. "</td><td>".$fetch['instructor']. "</td><td><a href='addcontent.php?id=".$id."'> Add Content </a></td><td> <a href='deletecourses.php?id=".$id."'> Delete Course </a> </td></tr>";
		            $i++;
		        }
		        echo "</table>";
		    } 
